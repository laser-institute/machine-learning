...
final_fit %>% 
    ...
    mutate(correct = .pred_class == good_grad_rate) # create a new variable, correct
```

You may notice some of the rows may be missing values. This is because there were some missing values in some of the predictor variables, and for this machine learning algorithm (the generalized linear model), missing values result in row-wise deletion.
