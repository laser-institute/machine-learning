...
```{python}
ipeds_df['good_grad_rate'] = np.where(ipeds_df['grad_rate'] > 62, 1, 0)
```
...
A short-cut to the above is to simply use the scikit-learn `accuracy_score()` function (which we already imported above), taking `y_test` as the first argument and `y_preds` as the second; **we'll use this short-cut from here forward**, having seen how accuracy is calculated. Write that code below.
