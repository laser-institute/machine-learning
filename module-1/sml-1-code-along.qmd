...
**Fit model -- we'll just use three variables to begin with a very simple model - how do several variables relate to a Pokemon being legendary**
...
model = smf.logit('is_legendary ~ height_m + weight_kg + hp', data=pokemon_df).fit()
```
...
pokemon_preds <- predict(log_reg_fit, pokemon, type = "class") %>%
  bind_cols(pokemon %>% select(is_legendary))

metrics(pokemon_preds, truth = is_legendary, estimate = .pred_class)
```
:::
